...
# start the model 'uvicorn main:app --reload'
...
app.add_middleware(
    CORSMiddleware,
    allow_origins=["http://localhost:5173", "https://gpdv375t-5173.asse.devtunnels.ms"],  # React dev server
...
def transform_user_input(user: UserProfile):
    ...
    # Assign ratings based on chosen skills
    ...
    data = {
        ...
        "CourseDuration": [user.CourseDuration if user.CourseDuration else "None"]  # added
    }
...
def predict(user: UserProfile):
    ...
    # Replace unseen categorical values with "None" before transforming
